...
"   - set callback when pipe is ready for read and ogoforce refresh()
...
class Explorer
  ...
  private
    ...
    def choose_d(open_mode)
      entry = @current_sorted_matches[@selected_index]
      return if entry.nil?
      del_entry(entry, open_mode)
    end
...
class BufferExplorer < Explorer
  ...
  private
    ...
    def open_entry(entry, open_mode)   #nubfunkcja
      cleanup()
  ...
  public
    ...
    def create(prefix)
      ...
      if VIM::has_syntax?
        ...
        VIM::command 'highlight link LustySelected lustyType'
